...
abstract class RepositoryImplementation
{
     ...
     * @return mixed
     */
    public function findByDataTable($key, $value, $operator = '=',$orderBy = 'desc')
    {
        $user = Auth::user();
        if($user->isSuperAdmin())
        {
            return $this->getModel()->query()->where($key, $operator, $value)->orderBy('id',$orderBy);
        }
        else {
    ...
    {
        if ($paginate) {
            return $this->getModel()->where($key, $operator, $value)->orderBy('id','desc')->paginate($this->perPage());
        } else {
