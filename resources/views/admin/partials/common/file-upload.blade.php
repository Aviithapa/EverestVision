...
<script>
    ...
    function anyCommonFileUploader(id) {
        $('#'+ id +'_progress').css('width','0%');
        $('#'+id).fileupload({
            url: '{{ url('dashboard/ajax/common-file-upload') }}' + '/' + id,
            done: function(e, data) {
                $('#'+id+'_path').val(data.result.id);
                @dd(('#'+id+'_path').val(data.result.id));
                $('#'+id+'_img').attr('src', data.result.full_url);
                $('#'+id+'_href').attr('href', data.result.full_url);
                $('#'+ id +'_progress').parent().removeClass('progress-striped');
                $('#'+id+'_help_text').text('File Upload Successfully');
            },
            error: function(e,data){
